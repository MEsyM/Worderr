...
- **Prompt sequencing** – Each room can include a stack of prompts that act as lyrical cues for subsequent turns. Hosts type one prompt per line when creating the room; the client converts the textarea input into an ordered list that is saved in the `prompts` array. During the session, the application surfaces the current prompt to keep collaborators aligned on the theme while preserving the remaining prompts for upcoming rounds.
